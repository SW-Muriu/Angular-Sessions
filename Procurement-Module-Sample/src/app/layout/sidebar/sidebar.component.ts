...
export class SidebarComponent implements OnInit, OnDestroy {
  ...
  constructor(
    ...
    this.routerObj = this.router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        ...
        const role = ["ROLE_ADMIN", "ROLE_USER", "ROLE_DESK", "ROLE_TECHNICAL", "ROLE_FINANCIAL", "ROLE_HEAD", "ROLE_SUPPLIER"];
  ...
  ngOnInit() {
    ...
    if (this.currentUser) {
      ...
      } else if (userRole === Role.Financial) {
        this.userType = Role.Financial;
      } else if (userRole === Role.Technical) {
        this.userType = Role.Technical;
      } else if (userRole === Role.Desk) {
        this.userType = Role.Desk;
      } else if (userRole === Role.HOD) {
        this.userType = Role.HOD;
      } else if (userRole === Role.ProcAdmin) {
        this.userType = Role.ProcAdmin;
      } else if (userRole === Role.Supplier) {
        this.userType = Role.Supplier;
      }
      else {
  ...
  logout(): void {
    if (this.currentUser.roles[0] === Role.Supplier) {
