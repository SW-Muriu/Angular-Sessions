    ...
    this.tenderAdvertService.fetchAllTenderAdverts()
      ...
      .subscribe(
        (result) => {
          ...
          if (this.tenderAdverts.length > 0) {
            this.rejectedTenderAdverts = [];

            this.tenderAdverts.forEach(tenderAdvert => {
              if(tenderAdvert.status === "Rejected")
              this.rejectedTenderAdverts.push(tenderAdvert);

              if(this.rejectedTenderAdverts.length > 0){
                this.isLoading = false;

                this.dataSource = new MatTableDataSource<any>(this.rejectedTenderAdverts);
