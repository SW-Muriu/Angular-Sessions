...
@Component({
  selector: "app-not-posted-announcements",
  templateUrl: "./not-posted-announcements.component.html",
  styleUrls: ["./not-posted-announcements.component.sass"],
})
export class NotPostedAnnouncementsComponent implements OnInit {
  fetchedData: any;
  ...
  ngOnInit(): void {
    ...
    this.getApprovedNotPostedAnnouncements();
  }
  ...
  getApprovedNotPostedAnnouncements() {
    this.announcementService.getApprovedNotPostedAnnouncements().subscribe(
      (res) => {
  ...
  updateAnnouncement(row) {
    this.router.navigate(["/procurement-admin/announce/update-announcement"], {
      state: {
  ...
  postAnnouncement(data) {
    const params = new HttpParams().set("referenceid", data.referenceid);
    this.announcementService.postAnnouncement(params).subscribe(
      (res) => {
        console.log("res: ", res);
        this.snackbar.showNotification("snackbar-success", "Announcement posted sucessfully!");
      },
      (err) => {
        console.log(err);
        this.snackbar.showNotification("snackbar-danger", "Error posting announcement!");
      }
    );
  }
}
