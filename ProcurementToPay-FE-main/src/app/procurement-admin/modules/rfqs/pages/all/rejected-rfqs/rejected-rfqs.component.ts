...
@Component({
  selector: 'app-rejected-rfqs',
  templateUrl: './rejected-rfqs.component.html',
  styleUrls: ['./rejected-rfqs.component.sass']
})
export class RejectedRfqsComponent extends BaseComponent implements OnInit {
          ...
          if (this.rfqs.length > 0) {
            ...
            this.rfqs.forEach((rfq) => {
              if (rfq.status == "Rejected") {
                this.rejectedRfqs.push(rfq);
              }
            });

            console.log("Results ", result);

            if (this.rejectedRfqs.length > 0) {
              this.isLoading = false;

              this.dataSource = new MatTableDataSource<any>(this.rejectedRfqs);
