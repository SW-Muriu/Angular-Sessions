...
export class NeedRequisitionFormComponent extends BaseComponent implements OnInit {
  currentUser: any;
  user: any
  username: string;
  userId: number;
...
specifyQuantity(event: any, i: any){
  ...
  getUserById(userId) {
    this.needRequisitionService.getUserDetailsById(userId)
      .pipe(takeUntil(this.subject))
      .subscribe(
        (res) => {
          this.user = res;
  ...
  requestNeed() {
    console.log("Cost center: ", this.needRequisitionForm.value);
    this.needRequisitionService.requestNeed(this.needRequisitionForm.value).subscribe(
      (res) => {
        ...
        this.router.navigate(['/departmental-users/need-management/pending-needs'])
      },
      (err) => {
        console.log(err);
      }
